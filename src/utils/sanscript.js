...
function exportSanscriptSingleton(global, schemes, devanagariVowelToMarks) {
  ...
  Sanscript.addRomanScheme = function (name, scheme) {
    if (!("vowel_marks" in scheme)) {
      scheme.vowel_marks = {};
      for (
        var _i = 0, _Object$entries = Object.entries(scheme.vowels);
  ...
  (function () {
    ...
    var addCapitalAlternates = function addCapitalAlternates(
      codeList,
      alternatesMap
    ) {
    ...
    addCapitalAlternates(
      Object.values(schemes.iast.vowels),
      schemes.iast.alternates
    );
    addCapitalAlternates(
      Object.values(schemes.iast.consonants),
      schemes.iast.alternates
    );
    addCapitalAlternates(
      Object.values(schemes.iast.extra_consonants),
      schemes.iast.alternates
    );
    addCapitalAlternates(["oṃ"], schemes.iast.alternates);
    addCapitalAlternates(
      Object.values(schemes.kolkata_v2.vowels),
      schemes.kolkata_v2.alternates
    );
    addCapitalAlternates(
      Object.values(schemes.kolkata_v2.consonants),
      schemes.kolkata_v2.alternates
    );
    addCapitalAlternates(
      Object.values(schemes.kolkata_v2.extra_consonants),
      schemes.kolkata_v2.alternates
    );
    addCapitalAlternates(
      Object.values(schemes.iso.vowels),
      schemes.iso.alternates
    );
    addCapitalAlternates(
      Object.values(schemes.iso.consonants),
      schemes.iso.alternates
    );
    addCapitalAlternates(
      Object.values(schemes.iso.extra_consonants),
      schemes.iso.alternates
    );
    addCapitalAlternates(["ōṁ"], schemes.iso.alternates);
    for (
      var _i2 = 0, _Object$entries2 = Object.entries(schemes);
  ...
  var makeMap = function makeMap(from, to, _options) {
    ...
    for (var group in fromScheme) {
      ...
      if (
        ["alternates", "accented_vowel_alternates", "isRomanScheme"].includes(
          group
        )
      ) {
        continue;
      ...
      for (
        var _i3 = 0, _Object$entries3 = Object.entries(fromGroup);
    ...
    if (fromScheme["accented_vowel_alternates"]) {
      for (
        var _i4 = 0,
          _Object$keys = Object.keys(fromScheme["accented_vowel_alternates"]);
        ...
        try {
          for (_iterator3.s(); !(_step3 = _iterator3.n()).done; ) {
            ...
            var baseVowel = baseAccentedVowel.substring(
              0,
              baseAccentedVowel.length - 1
            );
  ...
  var transliterateRoman = function transliterateRoman(data, map, options) {
    ...
    if (!toRoman && Object.keys(map.accents).length > 0) {
      var pattern = new RegExp(
        "(["
          .concat(Object.values(map.accents).join(""), "])([")
          .concat(Object.values(toScheme["yogavaahas"]).join(""), "])"),
        "g"
      );
  ...
  var transliterateBrahmic = function transliterateBrahmic(
    data,
    map,
    _options
  ) {
  ...
  Sanscript.t = function (data, from, to, options) {
    ...
    if (from === "tamil_superscripted") {
      var pattern =
        "([" +
        Object.values(schemes["tamil_superscripted"]["vowel_marks"]).join("") +
        schemes["tamil_superscripted"]["virama"]["्"] +
        "॒॑" +
        "]+)([²³⁴])";
      data = data.replace(new RegExp(pattern, "g"), "$2$1");
      console.error(
        "transliteration from tamil_superscripted not fully implemented!"
      );
    }
    ...
    if (_typeof(options.preferred_alternates[to]) == "object") {
      ...
      for (var i = 0; i < keys.length; i++) {
        result = result
          .split(keys[i])
          .join(options.preferred_alternates[to][keys[i]]);
      }
    }
    return result;
  };
